...
def adicionar_item_tarefa(tarefa) :
    frame_tarefa = tk.Frame(canvas_interior, bg='white', bd=1, relief ='solid')
    ...
    botao_editar = tk.Button(frame_tarefa, image=icon_editar, command =lambda f=frame_tarefa, l=label_tarefa: preparar_edicao(f, l), bg='white', relief='raised')
    botao_editar.pack(side=tk.RIGHT, padx=5)


    botao_deletar = tk.Button(frame_tarefa, image=icon_delete, command =lambda f=frame_tarefa: deletar_tarefa(f), bg='white', relief='raised')
...
def preparar_edicao(frame_tarefa, label_tarefa):
    global frame_em_edicao
    botao_adicionar.config(text='Salvar edição')
...
def atualizar_tarefa(nova_tarefa): 
    ...
    for widget in frame_em_edicao.winfo_children():
        if isinstance(widget, tk.Label):
            ...
            botao_adicionar.config(text='Adicionar Tarefa')
            entrada_tarefa.delete(0, tk.END)            
...
icon_delete = PhotoImage(file='delete.png').subsample(3, 3)


fonte_cabecalho = font.Font(family='Garamond', size=24, weight='bold')
rotulo_cabecalho = tk.Label(janela, text='Meu app de Tarefas', font=fonte_cabecalho, bg='white', fg='black').pack(pady=20)


frame = tk.Frame(janela, bg='white')
frame.pack(pady=10)
entrada_tarefa = tk.Entry(frame, font=('Garamond', 14),bd=2, relief='sunken', bg='white', fg='black', width=30)
entrada_tarefa.pack(side=tk.LEFT, padx=10)


botao_adicionar = tk.Button(frame, command=adicionar_tarefa, text='Adicionar Tarefa',bd=2, bg='#32CD32', fg='black', height=1, width=15, font=('Roboto', 11), relief='raised')
botao_adicionar.pack(side=tk.LEFT, padx=10)


#Criar um frame para a lista de tarefas de rolagem
